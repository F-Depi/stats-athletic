...
def get_data_from_url(url, folder_path):
    ...
    for table_index, table in enumerate(tables):
        ...
        if table_index % 2 == 0:
            ...
            if os.path.exists(f"{folder_path}/{csv_file_name}"):
                # The csv file already exists, so open it in append mode
                csvfile = open(f"{folder_path}/{csv_file_name}", "a", newline='', encoding='utf-8')
            else:
                # The csv file does not exist, so create it and write the header rows
                csvfile = open(f"{folder_path}/{csv_file_name}", "w", newline='', encoding='utf-8')
